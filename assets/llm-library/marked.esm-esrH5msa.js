...
function se(r, e) {
  let n = r.replace(x.findPipe, (i, l, o) => {
    let a = !1, h = l;
    for (; --h >= 0 && o[h] === "\\"; ) a = !a;
    return a ? "|" : " |";
...
function le(r, e, n, s, t) {
  let i = e.href, l = e.title || null, o = r[1].replace(t.other.outputLinkReplace, "$1");
  s.state.inLink = !0;
  let a = { type: r[0].charAt(0) === "!" ? "image" : "link", raw: n, href: i, title: l, text: o, tokens: s.inlineTokens(o) };
...
var v = class {
  ...
  code(r) {
    ...
    if (e) {
      ...
      return { type: "code", raw: e[0], codeBlockStyle: "indented", text: this.options.pedantic ? n : z(n, `
`) };
    }
  }
  fences(r) {
    ...
    if (e) {
      let n = e[0], s = Ye(n, e[3] || "", this.rules);
...
`), s = "", t = "", i = [];
      for (; n.length > 0; ) {
        let l = !1, o = [], a;
        for (a = 0; a < n.length; a++) if (this.rules.other.blockquoteStart.test(n[a])) o.push(n[a]), l = !0;
...
${c}` : c;
        ...
        if (this.lexer.state.top = !0, this.lexer.blockTokens(c, i, !0), this.lexer.state.top = k, n.length === 0) break;
        let u = i.at(-1);
...
` + n.join(`
`), b = this.list(f);
          i[i.length - 1] = b, s = s.substring(0, s.length - u.raw.length) + b.raw, t = t.substring(0, t.length - d.raw.length) + b.raw, n = f.substring(i.at(-1).raw.length).split(`
`);
          continue;
        }
      }
...
`, r = r.substring(u.length + 1), a = !0), !a) {
          let q = this.rules.other.nextBulletRegex(f), J = this.rules.other.hrRegex(f), V = this.rules.other.fencesBeginRegex(f), Y = this.rules.other.headingBeginRegex(f), xe = this.rules.other.htmlBeginRegex(f);
          for (; r; ) {
            let Z = r.split(`
`, 1)[0], S;
            if (u = Z, this.options.pedantic ? (u = u.replace(this.rules.other.listReplaceNesting, "  "), S = u) : S = u.replace(this.rules.other.tabCharGlobal, "    "), V.test(u) || Y.test(u) || xe.test(u) || q.test(u) || J.test(u)) break;
            if (S.search(this.rules.other.nonSpaceChar) >= f || !u.trim()) c += `
` + S.slice(f);
            else {
              if (d || k.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || V.test(k) || Y.test(k) || J.test(k)) break;
              c += `
` + u;
            }
            !d && !u.trim() && (d = !0), h += Z + `
`, r = r.substring(Z.length + 1), k = S.slice(f);
          }
        }
        t.loose || (l ? t.loose = !0 : this.rules.other.doubleBlankLine.test(h) && (l = !0));
        let b = null, K;
        this.options.gfm && (b = this.rules.other.listIsTask.exec(c), b && (K = b[0] !== "[ ] ", c = c.replace(this.rules.other.listReplaceTask, ""))), t.items.push({ type: "list_item", raw: h, task: !!b, checked: K, loose: !1, text: c, tokens: [] }), t.raw += h;
      }
      let o = t.items.at(-1);
      ...
      for (let a = 0; a < t.items.length; a++) if (this.lexer.state.top = !1, t.items[a].tokens = this.lexer.blockTokens(t.items[a].text, []), !t.loose) {
        let h = t.items[a].tokens.filter((k) => k.type === "space"), c = h.length > 0 && h.some((k) => this.rules.other.anyLine.test(k.raw));
        t.loose = c;
      }
...
`) : [], i = { type: "table", raw: e[0], header: [], align: [], rows: [] };
    if (n.length === s.length) {
      ...
      for (let l of t) i.rows.push(se(l, i.header.length).map((o, a) => ({ text: o, tokens: this.lexer.inline(o), header: !1, align: i.align[a] })));
      return i;
    }
  }
  lheading(r) {
...
` ? e[1].slice(0, -1) : e[1];
  ...
  emStrong(r, e, n = "") {
    ...
    if (!(!s || s[3] && n.match(this.rules.other.unicodeAlphaNumeric)) && (!(s[1] || s[2]) || !n || this.rules.inline.punctuation.exec(n))) {
      ...
      for (h.lastIndex = 0, e = e.slice(-1 * r.length + t); (s = h.exec(e)) != null; ) {
        ...
        if (o -= l, o > 0) continue;
        l = Math.min(l, l + o + a);
        let c = [...s[0]][0].length, k = r.slice(0, t + s.index + c + l);
        if (Math.min(t, l) % 2) {
          let d = k.slice(1, -1);
  ...
  inlineQueue;
  constructor(e) {
    this.tokens = [], this.tokens.links = /* @__PURE__ */ Object.create(null), this.options = e || R, this.options.tokenizer = this.options.tokenizer || new v(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = { inLink: !1, inRawBlock: !1, top: !0 };
...
`;
  ...
  textRenderer;
  constructor(e) {
    this.options = e || R, this.options.renderer = this.options.renderer || new B(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new X();
  }
  static parse(e, n) {
  ...
  parse(e, n = !0) {
    ...
    for (let t = 0; t < e.length; t++) {
      ...
      switch (l.type) {
        ...
        case "text": {
          let o = l, a = this.renderer.text(o);
          for (; t + 1 < e.length && e[t + 1].type === "text"; ) o = e[++t], a += `
` + this.renderer.text(o);
          n ? s += this.renderer.paragraph({ type: "paragraph", raw: a, text: a, tokens: [{ type: "text", raw: a, text: a, escaped: !0 }] }) : s += a;
          continue;
        }
        default: {
  ...
  parseMarkdown(r) {
    return (e, n) => {
      ...
      if (t.hooks && (t.hooks.options = t, t.hooks.block = r), t.async) return (async () => {
        let l = t.hooks ? await t.hooks.preprocess(e) : e, o = await (t.hooks ? await t.hooks.provideLexer() : r ? w.lex : w.lexInline)(l, t), a = t.hooks ? await t.hooks.processAllTokens(o) : o;
        t.walkTokens && await Promise.all(this.walkTokens(a, t.walkTokens));
        let h = await (t.hooks ? await t.hooks.provideParser() : r ? m.parse : m.parseInline)(a, t);
      ...
      try {
        ...
        let o = (t.hooks ? t.hooks.provideParser() : r ? m.parse : m.parseInline)(l, t);
...
g.Hooks = A;
g.parse = g;
var tt = g.options, rt = g.setOptions, nt = g.use, st = g.walkTokens, lt = g.parseInline, it = g, at = m.parse, ot = w.lex;
export {
  A as Hooks,
  w as Lexer,
  et as Marked,
